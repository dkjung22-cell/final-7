function setup() {
  // 1. 캔버스 및 배경 설정
  createCanvas(600, 400); // 가로 600px, 세로 400px 크기의 캔버스 생성
  background(2);          // 검은색에 가까운 아주 어두운 회색 배경 세팅

  // 2. 주황/빨간색 계열의 구조물 (화면 중앙~우측)
  fill("#FFA040");
  rect(350, 90, 10, 100); // 얇고 긴 주황색 직사각형
  fill("#FFC080");
  rect(350, 195, 9, 86);  // 아래쪽에 이어지는 연주황색 직사각형

  stroke("#FF9999");
  line(365, 40, 365, 70);   // 상단 연분홍색 세로선
  stroke("#FF9999");
  line(362, 70, 362, 289);  // 길게 내려오는 연분홍색 세로선

  fill("#C00000");
  quad(365, 70, 390, 120, 390, 280, 364, 280); // 짙은 빨간색의 비대칭 사각형 (입체감 표현)
  rect(330, 150, 16, 135); // 왼쪽에 위치한 붉은 계열 직사각형

  stroke("#FFC0C0");
  line(326, 130, 326, 300); // 연분홍색 세로선
  stroke("#FFC0C0");
  strokeWeight(5);          // 선 두께를 5px로 두껍게 설정
  line(320, 120, 320, 295); // 두꺼운 연분홍색 세로선

  // 3. 보라/살구색 계열의 구조물 (화면 우측)
  noStroke();               // 테두리선 없애기
  fill("#993399");
  rect(447, 200, 20, 85);  // 보라색 직사각형

  noStroke(0);
  fill("#FFCC99");
  quad(410, 170, 440, 165, 440, 280, 395, 280); // 살구색 비대칭 사각형

  stroke("#FFE5CC");
  strokeWeight(2);          // 선 두께 2px로 조절
  line(410, 130, 410, 310); // 연살구색 세로선
  stroke("#FFE5CC");
  line(415, 150, 415, 290); // 연살구색 세로선

  // 4. 중앙 좌측의 작은 구조물과 방울들 (주황/핑크 계열)
  rect(290, 230, 20, 48);   // 살구색 작은 직사각형
  stroke("#FF4040");
  line(310, 280, 310, 340); // 아래로 뻗는 빨간 세로선
  line(300, 280, 300, 320); // 아래로 뻗는 짧은 빨간 세로선

  fill("#FFE5FF");
  circle(310, 320, 13);     // 아주 연한 핑크색 원
  circle(300, 300, 7);      // 아주 연한 핑크색 작은 원

  // 5. 파란색 계열의 거대 구조물 (화면 중앙~좌측)
  fill("#4040FF");
  noStroke();
  quad(260, 200, 280, 180, 280, 290, 260, 290); // 파란색 비대칭 사각형

  stroke("#C0FFFF");
  line(285, 150, 285, 300); // 하늘색 세로선

  fill("#0000CC");
  noStroke();
  rect(220, 160, 37, 140);  // 짙은 파란색 직사각형
  noStroke();
  arc(239, 157, 40, 45, PI, TWO_PI); // 직사각형 위에 얹은 파란색 아치(반원) 모양

  stroke("#0000FF");
  line(239, 100, 239, 160); // 아치 위로 솟은 파란 세로선

  stroke("#99CCFF");
  strokeWeight(5);
  line(210, 80, 210, 330);  // 화면을 가로지르는 두꺼운 하늘색 세로선

  stroke("#C0FFFF");
  strokeWeight(2);
  line(216, 100, 216, 360); // 하늘색 세로선

  fill("#4040FF");
  noStroke();
  quad(150, 130, 200, 100, 200, 290, 150, 290); // 파란색 큰 비대칭 사각형

  stroke(200);
  line(195, 110, 195, 320); // 회색 세로선
  stroke("#CCCCFF");
  strokeWeight(3);
  line(180, 280, 180, 340); // 연보라빛 세로선

  noStroke();
  fill("#80C0FF");
  rect(135, 180, 10, 120);  // 하늘색 얇은 직사각형
  fill("#333399");
  rect(123, 150, 5, 160);   // 남색 아주 얇은 직사각형

  // 6. 양쪽 외곽의 미세한 세로선 및 구조물
  stroke("#CCCCFF");
  line(473, 180, 473, 290); // 우측 연보라색 세로선

  fill("#400080");
  noStroke();
  quad(100, 250, 119, 220, 119, 296, 90, 296); // 좌측 하단 진보라색 사각형

  stroke("#CCCCFF");
  line(115, 190, 115, 300); // 좌측 연보라색 세로선
  line(480, 230, 480, 300); // 우측 연보라색 세로선 1
  line(486, 210, 486, 290); // 우측 연보라색 세로선 2

  // 7. 배경을 장식하는 다양한 색상과 크기의 원들 (빛망울/별 효과)
  noStroke();
  fill("#FFCCCC");          // 연분홍색 원들
  circle(450, 90, 30);
  circle(430, 140, 5);
  circle(228, 80, 4);

  fill("#CCE5FF");          // 연하늘색 원들
  circle(281, 136, 3);
  circle(310, 110, 6);
  circle(120, 100, 15);

  fill("#E5E5FF");          // 연보라색 원들
  circle(160, 320, 7);
  circle(450, 340, 15);
  circle(430, 320, 3);
  circle(450, 310, 5);

  fill("#FF9999");          // 분홍색 원들
  circle(500, 190, 7);
  circle(80, 200, 4);

  fill("#99CCFF");          // 파란색 원
  circle(260, 50, 13);

  // 8. 우측 외곽의 마지막 하늘색조 구조물
  noStroke();
  fill("#C0E0FF");
  rect(490, 250, 10, 40);   // 하늘색 작은 직사각형
}